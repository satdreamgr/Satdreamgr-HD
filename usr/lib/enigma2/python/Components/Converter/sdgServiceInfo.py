...
class sdgServiceInfo(Converter, object):
    ...
    def getText(self):
        ...
        if self.type == self.SERVICENAME:
            ...
            if config.plugins.sdgServiceInfo.showServiceNumber.value == True and number != '':
                text = '%s. %s' % (number, name)
            else:
                text = name
            if config.plugins.sdgServiceInfo.showOrbitalPosition.value == True and orbital != '':
                if config.plugins.sdgServiceInfo.orbitalPositionType.value == 'name':
